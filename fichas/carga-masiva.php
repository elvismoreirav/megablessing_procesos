<?php
/**
 * Carga masiva de fichas de recepcion desde archivo XLSX.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors) && strtolower(pathinfo($fileName, PATHINFO_EXTENSION)) !== 'xlsx') {
...
$pageTitle = 'Carga Masiva de Recepcion';
$pageSubtitle = 'Importe fichas de recepcion desde una plantilla XLSX';
...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex items-center gap-2 text-sm text-warmgray">
        <a href="<?= APP_URL ?>/fichas/index.php?vista=recepcion" class="hover:text-primary">Recepcion</a>
    ...
    <div class="card">
        <div class="card-body">
            <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
                <div class="space-y-3">
                    <h2 class="text-xl font-semibold text-gray-900">Subir fichas de recepcion desde Excel</h2>
                    <p class="text-gray-600 max-w-3xl">
                        Descargue la plantilla oficial, complete la hoja <strong>Recepcion</strong> y suba el archivo para crear las fichas.
                        Si <code>lote_codigo</code> queda vacio, el sistema crea el lote automaticamente con proveedor, variedad, fecha y calidad.
                    </p>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-gray-600">
                        <div class="bg-amber-50 border border-amber-100 rounded-xl p-4">
                            <p class="font-medium text-amber-800">Campos obligatorios</p>
                            <p class="mt-1"><code>proveedor_codigo</code>, <code>variedad_codigo</code>, <code>tipo_entrega</code>, <code>fecha_entrada</code>, pesos, revision visual, calidad y clasificacion.</p>
                        </div>
                        <div class="bg-blue-50 border border-blue-100 rounded-xl p-4">
                            <p class="font-medium text-blue-800">Multiples fichas por lote</p>
                            <p class="mt-1">Puede repetir el mismo <code>lote_codigo</code> en varias filas para crear varias fichas de recepcion del mismo lote.</p>
                        </div>
                    </div>
                </div>
                <div class="flex flex-col sm:flex-row lg:flex-col gap-3 lg:min-w-[250px]">
                    <a href="<?= APP_URL ?>/fichas/formato-carga-masiva.php" class="btn btn-outline">
    ...
    <?php if ($result !== null): ?>
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
            ...
            <div class="stat-card accent">
                <p class="text-white/80 text-sm">Creadas</p>
                <p class="text-3xl font-bold mt-1"><?= number_format($result['totals']['created']) ?></p>
                <p class="text-white/70 text-sm mt-2">Fichas registradas correctamente</p>
            </div>
            <div class="stat-card gold">
                <p class="text-primary-dark/80 text-sm">Repetidas</p>
        ...
        <?php if ($result['totals']['created'] > 0): ?>
            <div class="alert alert-success">
                ...
                <div>
                    ...
                    <p class="mt-1">Se registraron <?= number_format($result['totals']['created']) ?> fichas de recepcion nuevas.</p>
                </div>
            </div>
        <?php endif; ?>
        ...
        <?php if (!empty($result['created'])): ?>
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Fichas creadas</h3>
                </div>
                        ...
                        <tbody>
                            <?php foreach ($result['created'] as $createdItem): ?>
                                <tr>
                                    ...
                                    <td>#<?= (int)$createdItem['ficha_id'] ?></td>
                                    <td class="font-semibold text-primary"><?= htmlspecialchars($createdItem['lote_codigo']) ?></td>
                                    <td><?= htmlspecialchars($createdItem['proveedor']) ?></td>
                                    <td><?= Helpers::formatDate($createdItem['fecha_entrada']) ?></td>
                                    <td><?= number_format((float)$createdItem['peso_final_registro'], 2, ',', '.') ?> <?= htmlspecialchars($createdItem['unidad_peso']) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
        ...
        <?php foreach (['duplicates' => 'Filas repetidas', 'errors' => 'Errores detectados'] as $key => $title): ?>
                ...
                <div class="card">
                    ...
                    <div class="table-container">
                        <table class="table">
                            ...
                            <tbody>
                                <?php foreach ($result[$key] as $item): ?>
                                    <tr>
